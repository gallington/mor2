...
td<- mor2FULL%>% dplyr::select(ResWint = a_ResWint,  # Practice
                  TenurePast = q02a_RightNatWintPast,  # Tenure on Winter Pasture
                  UseCamp    = A_UsWtrCamp,         # Have or Use a winter Campsite
                  TenureCamp = B_ContractWtrCamp,   # Have a use or posseion contract for the winter camp
...
td %<>% mutate_at(2:9, funs(factor(.)))
td$Rule<- ordered(td$Rule)
```

  # ADD RECODED Winter Camp Tenure. 
# need to recode the Winter Camp data using 2 diff vars, so that it is No don't use, Yes use but don't have a contract or Yes use and do have a use or poassession contract: 
```{r}
...
tm1 <- glmer(ResWint ~ TenurePast + (1|ez), family = binomial,data = td)
tm2 <- glmer(ResWint ~ TenurePast + (1|ez) + (1|cbrmType), family = binomial,data = td)
tm3 <- glmer(ResWint ~ TenurePast + Rule + (1|ez), family = binomial,data = td)
tm4 <- glmer(ResWint ~ TenurePast + Rule + (1|ez) + (1|cbrmType), family = binomial,data = td)
tm5 <- glmer(ResWint ~ TenurePast + Rule + (ez|Rule), family = binomial,data = td)
tm6 <- glmer(ResWint ~ TenurePast + Rule + cbrmYN + (1|ez) , family = binomial,data = td) # 2nd BEST SO FAR
tm7 <- glmer(ResWint ~ TenurePast + cbrmYN + (1|ez) , family = binomial,data = td)   # BEST SO FAR
# How to tell if this is any better??? 
tm8 <- glmer(ResWint ~ TenurePast + cbrmYN + (1|ez) + (cbrmYN|cbrmType), family = binomial,data = td) 
```
...
```{r, message = FALSE, warning= FALSE}
#Better to use lmer() for mixed effects ...
cm1 <- glmer(ResWint ~ WtrCamp + (1|ez), family = binomial,data = td)
cm2 <- glmer(ResWint ~ WtrCamp + (1|ez) + (1|cbrmType), family = binomial,data = td)
cm3 <- glmer(ResWint ~ WtrCamp + Rule + (1|ez), family = binomial,data = td)
# tm5 <- glmer(ResWint ~ WtrCamp + Rule + (ez|Rule), family = binomial,data = td)

  # add tenure on pastures AND winter camp in one model: 
cm4 <- glm(ResWint ~ TenurePast + WtrCamp, family = binomial,data = td)
cm5 <- glm(ResWint ~ TenurePast + WtrCamp + Rule , family = binomial,data = td)
cm6 <- glmer(ResWint ~ TenurePast + WtrCamp + cbrmYN + (1|ez), family = binomial,data = td)
cm7 <- glmer(ResWint ~ TenurePast + WtrCamp + Rule + cbrmYN + (1|ez), family = binomial,data = td)
