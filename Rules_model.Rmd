...
library(lme4)
```
# NEXT UP : making dummy vars for use and contract vars.
...
t2c <- glmer(ResWint ~ hhTenureWPast + hhTenureWCamp + (1|ez), family = binomial,data = td)
summary(t1)
```
...
```{r}
#
sp.plots<- tab_model(htmSp, htmSp.1)
sp.plots
```
...
mtm <- glmer(ResWint ~  hhTenureWCamp+ RuleNo + cbrmYN + cogSC1 + (1|Org) , data = mfs, family = "binomial")

dsm <- glmer(ResWint ~ TenureWPast + Rule + cbrmYN + cogSC1 + (1|Org), data = ds, family = "binomial")
stm <- glmer(ResWint ~ TenureWPast + Rule + cbrmYN + cogSC1 , data = st, family = "binomial")
