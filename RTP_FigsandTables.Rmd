...
pv <- c(
  ...
  othersIn = "Access to Others' Pastures\n in Same Soum",
  otherOut = "Access to Others' Pasture\n in a Different Soum"
)
...
#FIGURE 2 
Figures 2 and 3 are currently in radar_plot.R
#FIGURE 3
...
woscop <- ggplot(wodat, aes( x= x, y = predicted, group = Rules)) +
  geom_smooth(aes(linetype = Rules), method = lm, se = FALSE, color = "black")+
  facet_wrap(vars(otherPast))+
  labs(x ="Structural Social Capital", 
       y = "Predicted probability",
       title = "b)  Winter otor")
...
# 
ggsave("./output_figs/Fig4a.fotorbyaccessandrules.png", foscop, width = 6, height = 3, units = "in")
ggsave("./output_figs/Fig4b.wotorbyaccessandrules.png", woscop, width = 6, height = 3, units = "in")
# 
# 
...
rsez<- ggplot(rse.df, aes(x= cogSC1, y=PredProb, color= Rules )) +
  #geom_smooth()+  
  geom_line()+
...
ggsave("./output_figs/predRulelevel.png", rsez, width = 7, height = 4, units = "in")
```
...
#FIGURE Supplement # :  Access x social capital
