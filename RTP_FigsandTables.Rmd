...
wcdat<- get_model_data(hypwR, type = "pred", terms = c("cogSC1", "Rule", "hhTenureWCamp"))

sdat<- get_model_data(hypsR, type = "pred", terms = c("cogSC1", "Rule", "hhTenureSpPast"))
scdat<- get_model_data(hypsR, type = "pred", terms = c("cogSC1", "Rule", "hhTenureSpCamp"))
...
wsctr <- ggplot(wdat, aes( x= x, y = predicted, group = Rules)) +
  ...
  labs(x ="Cognitive Social Capital", 
       y = "Prob of reserving\n winter pasture",
       title = "a)  Tenure on winter pasture")

wcsctr <- ggplot(wcdat, aes( x= x, y = predicted, group = Rules)) +
  geom_smooth(aes(linetype = Rules), method = lm, se = FALSE, color = "black")+
  facet_wrap(vars(hhTenureWCamp))+
  theme_bw()+
  labs(x ="Cognitive Social Capital", 
       y = "Prob. of reserving\n winter pasture",
       title = "b)  Tenure on winter camp")

probbwpast<- plot_grid(wsctr, wcsctr, ncol = 1)
...
ssctr <- ggplot(sdat, aes( x= x, y = predicted, group = Rules)) +
  ...
  labs(x ="Cognitive Social Capital", 
       y = "Prob of reserving\n spring pasture",
       title = "c)  Tenure on spring pasture")

scsctr <- ggplot(scdat, aes( x= x, y = predicted, group = Rules)) +
  geom_smooth(aes(linetype = Rules), method = lm, se = FALSE, color = "black")+
  facet_wrap(vars(hhTenureSpCamp))+
  theme_bw()+
  labs(x ="Cognitive Social Capital", 
       y = "Prob of reserving\n spring pasture",
       title = "d)  Tenure on spring camp")

probspast<- plot_grid(ssctr, scsctr, ncol = 1)

# why does this version look different -- not linear. gah.
# plot_model(hypw, type = "pred", terms = c("cogSC1",  "Rule", "hhTenureWPast"))

ggsave("./output_figs/Fig2ab.wpast_sctr.png", probbwpast, 
       width = 7, height = 7, units = "in")

ggsave("./output_figs/Fig2cd.spast_sctr.png", probspast, 
       width =7, height = 7, units = "in")
...
```
#FIGURE 4
Predicted Rule level based on social capital & ez
```{r}
library(MASS)
# bw theme
set_theme(
...
#FIGURES 5 - 8 
Magnitude of the marginal effects of the predictors for each practice vary across the ecological zones:
These are created in marg_eff_by_ez.R script
each subplot is created separately. Then combined in a google slides doc
Would be better to find a way to plot them nicely here but it gets complicated bc of the diff legends.

#FIGURE 9
This is Maria's conceptual figure of the results. Created in ppt.



#FIGURE S1 :  Access x social capital

```{r}

# why did I make a new df here??? oh, to remove the Other col? 
# yeah that's why. ok whatever.
...
# changed to just use the td.fg df
accessAOV<- aov(cogSC1 ~ otherPast, data = td.op) # data = td.op)
summary(accessAOV)
...
ggsave("./output_figs/FigS1.accessOtherxCSC.png", aop, width = 4, height = 3, units = "in")
...
ggsave("./output_figs/FigS1.accessOtherxSSC.png", bcaop, width = 4, height = 3, units = "in")

```

#FIGURE S2 : OR plots of indiv ez models
located in margh_eff_by_ez.R

Potentially swap these for Figs 5-8 and put those in the Supplement?


#TABLE SX :
each individual ez model
