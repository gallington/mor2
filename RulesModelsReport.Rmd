...
```{r, echo = FALSE}
load("./data/td.RData")
  # eventually merge this in to the RM-datapull script so we don't have two diff ones, since we want forage now anyway.
...
1/3: Realized we will have an easier time estimating probabilities while holding params constant if just use the DVs. And going to use (what was) RuleNo and RuleFormal, bc RuleNo = >0 and RuleFormal = >1
But Changed the name in the df to RuleYes


# Summary of results for WPasture:
...
So the formula for Reserving winter pastures is: 
...
The odds of reserving are=
exp(everything above)
But can convert that to probability of reserving with:
p= 1/(1+exp(-(everything above)))

calculate a new column in the dataframe for the estimated probability of each value of X1 using the mean of X2 with the formula:
...
#get the means of each CONTINUOUS predictor variable and write to a variable
# mean.SC<- mean(td.fg$cogSC1)  # what is wrong w this? why can't i get a mean?
sc<-td.fg$cogSC1[!is.na(td.fg$cogSC1)]
mean.sc = mean(sc)  # calculated in xcel bc kept getting error here, issue w data type?
mean.frg<- mean(td.fg$frg.left)
mean.CV<- mean(td.fg$frgCV)

summary(hypw)
#calculate the prob for reserving pastures for each param when the other predictor params are held constant at mean or set to zero....
      ## adjust the code and var name below to get all the diff iterations
probRes_0 = 1/(1+exp(-(-6.059621 + 
                         (0.662 * 0) + # hold everything off/constant except psture Tenure
                         (0.518 * 0) +  # camp tenure
                         (1.308847 * 0) + # Rules Yes
                         (-0.215351 * 0) + # Rules Formal
                         (0.812 * mean.sc) +  # cogSC
                         (0.0475 * mean.frg) +  #frg.lef
                         (0.0462 * mean.CV))))   #frgCV

binprobs<- as.data.frame(cbind(probRes_0, probRes_tp, probRes_tpc, probRes_tc, probRes_ry, probRes_rf))
probs<- gather(binprobs, "Vars", "Probability", 1:6)
probs %<>% mutate(Varibale =c("Base", "Pasture Tenure", "Pasture & Camp", "Camp Tenure", "Rules", "Formal Rule")) %>% select(-Vars) %>% dplyr::mutate_if(is.numeric, format, 1)

contprobs<- as.data.frame(probRes_sc, probRes_f, probRes_fcv) 


td2<- td.fg %>%
  # adds a new col= pred prob of reserving, when varying SC, but all others held at 0 or mean
  mutate(prob.sc = 1/(1+exp(-(-6.059621 +     
                         (0.662 * 0) + # tenure
                         (0.518 * 0) +  # camp tenure
                         (1.308847 * 0) + # Rules Yes
                         (-0.215351 * 0) + # Rules Formal
                         (0.812 * td.fg$cogSC1) +  # cogSC
                         (0.048 * mean.frg) +  #frg.lef
                         (0.046 * mean.CV))))   #frgCV)
        ) %>%
    mutate(prob.frg = 1/(1+exp(-(-6.059621 +     
                         (0.662 * 0) + # tenure
                         (0.518 * 0) +  # camp tenure
                         (1.308847 * 0) + # Rules Yes
                         (-0.215351 * 0) + # Rules Formal
                         (0.812 * mean.sc) +  # cogSC
                         (0.048 * frg.left) +  #frg.lef
                         (0.046 * mean.CV))))   #frgCV)
        )%>%
    mutate(prob.fcv = 1/(1+exp(-(-6.059621 +     
                         (0.662 * 0) + # tenure
                         (0.518 * 0) +  # camp tenure
                         (1.308847 * 0) + # Rules Yes
                         (-0.215351 * 0) + # Rules Formal
...
```
## Probability of Reserving Winter Pastures, by individual parameter 
...
```{r}
#plot probailities when others are held constant
g.prob.SC<-ggplot(td2, aes(x=cogSC1, y=prob.sc)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "cognitive social capital",
       y = "probability of reserving winter pastures")+
  ggtitle("Prob of Rsv WPast given varied social capital (all other params constant)") +
...
```{r}
#plot probailities when others are held constant
g.prob.f<-ggplot(td2, aes(x=frg.left, y=prob.frg)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "Forage remaining (%)",
       y = "Probability of reserving winter pastures")+
  ggtitle("Prob of Rsv WPast given amt of forage remaining (all other params constant)") +
  theme(plot.title = element_text(size=8))+
  theme(axis.title = element_text(size=10))
#g.prob.f
```
...
```{r}
#plot probailities when others are held constant
g.prob.fcv<-ggplot(td2, aes(x=frgCV, y=prob.fcv)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "CV of Available forage (%)",
       y = "Probability of reserving winter pastures")+
  ggtitle("Prob of Rsv WPast given CV of forage (all other params constant)") +
  theme(plot.title = element_text(size=8))+
  theme(axis.title = element_text(size=10))
#g.prob.fcv
```
...
library(gridExtra)
grid.arrange(g.prob.SC, g.prob.f, g.prob.fcv, ncol=3, widths = c(6,6,6))
```




Plotting fitted values vs. predicted values..... 
Why do the predicted curves start so much higher than fitted?? bc responding to unrealistic set
of data ranges in the prediction dataset?
```{r, cache = TRUE, echo = FALSE}
#----- 
# df of params for prediction:
sc<- seq(0,2, .1)
p_dat <- cbind(expand.grid(hhTenureWPast=c(0,1),       #seq(0,1,1),
                           hhTenureWCamp=c(0,1),       #seq(0,1,1),
                          # otherPast = c(1,2,3),
                           RuleYes=c(0,1),
                           RuleFormal=c(0,1),
                           #ez = c(1,2,3,4),             #seq(1,4,1),
                           cogSC1= seq(0,2, .1),
                           frg.left = seq(10,90, 5),
                           frgCV = seq(10,90,5),
                           Org = (1)
                           ))

p_dat %<>% mutate_at(c(1:4,8), funs(factor(.)))
#p_dat %<>% mutate_at(3, funs(ordered(.)))

wfit<- data.frame(hypw@frame, fitted = fitted(hypw))

wfPred<- predict(hypw, p_dat, type="response", allow.new.levels = TRUE)
predwfDat <- data.frame(p_dat, wfPred)


ggplot(td.fg, aes(x= frg.left, color = ResWint))+geom_density()
a<- ggplot(wfit, aes(x= cogSC1, y = fitted, color = RuleYes))+
  geom_point()+
  geom_smooth(method = "glm", method.args = list(family = "quasibinomial"),  alpha=0.3)
b<- ggplot(predwfDat, aes(x= cogSC1, y = wfPred, color = RuleYes))+
  #geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "quasibinomial"),  alpha=0.3)+
  ylim(0,1)
grid.arrange(a,b, ncol=2, widths = c(6,6))

# grid by Tenure & ecological zone
ggplot(wfpred, aes(x= cogSC1, y = fitted, color = Rule))+
  geom_smooth()+
...
# SPRING PASTURES
Model based on hypothesized signif relationships, compared to the prior best-fit models
full model w/ all params in the Hyp table:
```{r}
fulls<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + Rule+  cogSC1 +bondSC+otherPast+ frgCV + frg.left+   (1|Org) , family = binomial, data = td.fg) 
fulls1<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + RuleNo + RuleInf+ RuleFormal+  cogSC1 +bondSC+otherPast+ frgCV + frg.left+   (1|Org) , family = binomial, data = td.fg) 
summary(fulls)
```

```{r}
hyps<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + Rule +  cogSC1 + frgCV + frg.left+   (1|Org) , family = binomial, data = td.fg) 
summary(hyps)
```
Prior best-fits (w/ and w/o forage use). These used the ordered Rule var
```{r}
  # Best without forage use
sm1<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + RuleYes +  cogSC1+   (1|Org) , family = binomial, data = td.fg) 
  #BEST w forage use:
sfm1<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + RuleYes + RuleFormal+  cogSC1 + frg.left+ frgCV+  (1|Org) , family = binomial, data = td.fg)  
sfm2<- glmer(ResSpr ~ hhTenureSpPast + hhTenureSpCamp + Rule +  cogSC1 + frg.left+ frgCV+  (1|Org) , family = binomial, data = td.fg)  
anova(sfm1, sfm2)
```

Comparison of model results across these four parameterizations:
```{r}
tab_model(fulls, hyps,    
          show.icc = TRUE,
          show.aic = TRUE)


plot_model(fulls, hyps,
...
fullwo2<- glmer(Wotor ~ hhTenureWPast + hhTenureWCamp + RuleNo+ RuleFormal+  cogSC1 +bondSC+otherPast+ frgCV + frg.left+   (1|Org) , family = binomial, data = td.fg) 
summary(fullwo)
fullfo<- glmer(Fotor ~ hhTenureWPast + hhTenureWCamp + Rule+  cogSC1 +bondSC+otherPast+ frgCV + frg.left+  (1|Org) , family = binomial, data = td.fg) 
summary(fullfo)
```
Hyp model:
```{r}
...
Winter otor:
```{r}
tab_model(fullwo, hypwo, show.aic = TRUE)
```
Fall otor:
```{r}
tab_model(fullfo, hypfo, show.aic=TRUE)
```
...
```{r}
tab_model(nullw, nullw1, wp, show.icc = TRUE, show.aic = TRUE)
```
Reserving Spring Pastures:
```{r}
tab_model(nullSp, nullSp1, sp, show.icc = TRUE, show.aic = TRUE)

```
Winter Otor:
```{r}
tab_model(nullwo, nullwo1, wo, show.icc = TRUE, show.aic = TRUE)

```

Fall Otor:
```{r}
tab_model(nullfo, nullfo1, fo, show.icc = TRUE, show.aic = TRUE)
