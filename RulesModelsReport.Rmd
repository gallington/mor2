...
Predicting probabilities for winter reserv
```{r, cache = TRUE}
...
Plotting fitted values vs. predicted values based on mock data..... 
Why do the predicted curves start so much higher than fitted?? bc responding to unrealistic set of data ranges in the prediction dataset?
...
#summary(hypw)
  ...
  #calculate the prob for reserving pastures for each param when the other predictor params are   #held constant at mean or set to zero....
      ## adjust the code and var name below to get all the diff iterations
probResS_fcv = 1/(1+exp(-(-5.417732 + 
                         (0.691647 * 0) + # psture Tenure
                         (0.571525 * 0) +  # camp tenure
                         (0.383931 * 0) + # Rules Yes
                         (0.430766 * 0) + # Rules Formal
                         (0.691976 * mean.sc) +  # cogSC
                         (0.035356 * mean.frg) +  #frg.lef
                         (0.055061 * td.fg$frgCV))))   #frgCV

binprobs<- as.data.frame(cbind(probResS_0, probResS_tp, probResS_tpc, probResS_tc, probResS_ry, probResS_rf))
probs<- gather(binprobs, "Vars", "Probability", 1:6)
probs %<>% mutate(Varibale =c("Base", "Pasture Tenure", "Pasture & Camp", "Camp Tenure", "Rules", "Formal Rule")) %>% select(-Vars) %>% dplyr::mutate_if(is.numeric, format, 1)
...
td3<- td.sc %>%
  # adds a new col= pred prob of reserving, when varying SC, but all others held at 0 or mean
  mutate(prob.sc = 1/(1+exp(-(-5.417732 + 
                         (0.691647 * 0) + # psture Tenure
                         (0.571525 * 0) +  # camp tenure
                         (0.383931 * 0) + # Rules Yes
                         (0.430766 * 0) + # Rules Formal
                         (0.691976 * sc) +  # cogSC
                         (0.035356 * mean.frg) +  #frg.lef
                         (0.055061 * mean.CV))))   #frgCV
        ) %>%
    mutate(prob.frg = 1/(1+exp(-(-5.417732 + 
                         (0.691647 * 0) + # psture Tenure
                         (0.571525 * 0) +  # camp tenure
                         (0.383931 * 0) + # Rules Yes
                         (0.430766 * 0) + # Rules Formal
                         (0.691976 * mean.sc) +  # cogSC
                         (0.035356 * td.sc$frg.left) +  #frg.lef
                         (0.055061 * mean.CV))))   #frgCV
        )%>%
    mutate(prob.fcv = 1/(1+exp(-(-5.417732 + 
                         (0.691647 * 0) + # psture Tenure
                         (0.571525 * 0) +  # camp tenure
                         (0.383931 * 0) + # Rules Yes
                         (0.430766 * 0) + # Rules Formal
...
```{r}
#plot probailities when others are held constant
g.prob.SC<-ggplot(td3, aes(x=cogSC1, y=prob.sc)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "cognitive social capital",
       y = "probability of reserving Spring pastures")+
  ggtitle("Prob of Rsv Spr Past (all other params constant)") +
  theme(plot.title = element_text(size=8))+
  theme(axis.title = element_text(size=10))
#g.prob.SC
```
...
```{r}
#plot probailities when others are held constant
g.prob.f<-ggplot(td3, aes(x=frg.left, y=prob.frg)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "Forage remaining (%)",
...
```{r}
#plot probailities when others are held constant
g.prob.fcv<-ggplot(td3, aes(x=frgCV, y=prob.fcv)) +
  geom_point() +
  geom_smooth(method = "glm", method.args = list(family = "binomial"), se = FALSE, alpha=0.3)+
  ylim(0,1)+
  labs(x = "CV of Available forage (%)",
...
```{r}
hypwort<- glmer(Wotor ~ hhTenureWPast+ hhTenureWCamp+bondSC+ otherPast+ RuleYes + RuleFormal+ frgCV + frg.left+ (0*ez)+ (1|Org) , family = binomial, data = td.fg) 
...
```{r}
tab_model(fullfo, hypfor, show.aic=TRUE)
plot_model(fullfo)
plot_model(hypfor)
