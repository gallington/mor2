...
full model, with two diff versions of the Rule parameter, either DV or ordinal:
```{r}
fullw<- glmer(ResWint ~ hhTenureWPast + 
                hhTenureWCamp + 
                RuleYes + 
               # RuleInf+ 
                RuleFormal+  
...
fullw1<- glmer(ResWint ~ hhTenureWPast + 
                 hhTenureWCamp + 
                 Rule+  
                 cogSC1 +
                 bondSC+
...
wm1<- glmer(ResWint ~ hhTenureWPast + hhTenureWCamp + Rule +  cogSC1 +   (1|Org) , family = binomial, data = td.fg) 
  #BEST w forage use:
wfm1<- glmer(ResWint ~ hhTenureWPast + hhTenureWCamp + Rule +  cogSC1 + frg.left+ frgCV+  (1|Org) , family = binomial, data = td.fg)  
```
...
calculate a new column in your dataframe for the estimated probability of each value of X1 using the mean of X2 with the formula:
...
wfpred<- data.frame(hypw@frame, fitted = fitted(hypw))

#predict(finalW, p_dat, type="response")
wfPred<- predict(hypw, p_dat, type="response")
predwfDat <- data.frame(p_dat, wfPred)
# fit<- tbl_df(fitted(wfmod))
# colnames(fit)<- c("fittedProp")
# fitDat<- bind_cols(td.fg, fit)



ggplot(td.fg, aes(x= frg.left, color = ResWint))+geom_density()
#ggplot(td.fg, aes(x= frg.left))+geom_histogram()
