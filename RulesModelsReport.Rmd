...
# Table of Variables for analysis
```{r results = "asis"}
#stargazer::stargazer(td, type = "html", title = "Table 1. Variables included in analysis")

#knitr::kable(td, type = "html", title = "Table 1. Variables included in analysis")
```
...
```{r}
# with this method we can compare across all three:
# by creating a flat version of the contingency tables
hh<- xtabs(~td$hhTenureWPast + td$hhTenureWCamp +ez , data = td)
ftable(hh) # %>% print(method= "render")

library(MASS)
loglm(~td$hhTenureWPast + td$hhTenureWCamp + ez, hh)


```


#Cross tabs between tenure on the pastures and tenure on the camp and comparison across ecological zones.
We created contingency tables and used Chi-Square tests of assocition(independence) to assess whether there is an association between the responses. 
(Ho = the variables are independent/not associated with each other) 

###My initial take: There is little correspondence between the tenure status on winter pastures and winter camps; there is not a similar pattern in response. 

##The actual test result: (We can reject the null hypothesis that they are independent : Evidence suggests that there *is* an association between the tenure status on winter camps and winter pastures....Χ2 = 213.5572   df = 4   p = .0000)
So, how do we actually interpret this?  Does this mean that they are not indepdendent and we shouldn't be considering them both in the model?

###My inital take: There was not an even distribution of the tenure status on pastures across th eecological zones. Most households (68%) reported that they did not have formal use (or possession) agreements. Those that did were relatively evenly distributed across the ecological zones.In cntrast, the majority of respondents (64.4%) did have formal use agreements/contracts for their winter camp.

##Actual test results: Evidence suggests that there *is* an association between ecological zone and tenure status? (PASTURE: Χ2 = 35.5196   df = 6   p = .0000) (CAMP: Χ2 = 65.2532   df = 6   p = .0000) Which is what we would expect/what we predicted.


Why are we getting diff test stats w diff chisq methods? Need to figure out what's under the hood in the ctable test...
bc it is also using diff df. So, I think I might need to just use the chisq.test() function.

```{r}
#summary(hh)
#freq(tobacco[ ,c("gender", "age.gr", "smoker")])

# I think this function will only let you cross two at a time... 
ctable(td$hhTenureWPast, td$hhTenureWCamp, chisq = TRUE, headings = FALSE) %>%
  print( method = "render")

# comparing stats w diff chisq methods:
ctestab<-table(td$hhTenureWCamp, td$hhTenureWPast)
print(chisq.test(ctestab))

ctable(td$hhTenureWPast, td$ez, chisq = TRUE, headings = FALSE) %>%
  print( method = "render")

ctestab<-table(td$ez, td$hhTenureWPast)
print(chisq.test(ctestab))

ctable(td$hhTenureWCamp, td$ez, chisq = TRUE, headings = FALSE) %>%
  print( method = "render")

ctestab<-table(td$ez, td$hhTenureWCamp)
print(chisq.test(ctestab))
#chisq.test(ctestab, simulate.p.value = TRUE, B = 1000)

```
Cross tabs between Reserving Pasture and tenure status:


NOTE: I get diff test stats for these comparisons when I run it w the built-it ctable chisq test versus using chisq.test().
significance levels are approx the same but not exactly and the test stat values are diff.
```{r}
# ot<- xtabs(~ResWint + hhTenureWPast, data = td)
# ftable(ot)
# summary(ot)
# ctable(td$ResWint, td$hhTenureWPast,  chisq = TRUE, headings = FALSE) %>%
#   print( method = "render")


ctestab<-table(td$ResWint, td$hhTenureWPast)
print(chisq.test(ctestab, simulate.p.value = TRUE, B= 1000))
```

There was a significant association between Tenure on the winter pasture and whether or not a household reported that they reserve the pasture (Χ2 = 8.8199,  p = .0006). [if run w monte carlo sims then don't get a df param]



Visualizations of the relationship btwn social capital and probability of reserving pastures, given diff levels of Rule formality:
...
```{r}
#This isn't really all that helpful, since cogSC is continuous....
sr <- xtabs(~ ResWint + Rule + ez, data =td)
ftable(sr)

#freq(td[,c("ResWint", "Rule", "ez")], style = "rmarkdown")
```
...
# ftable(rc)
chisq.test(rc, simulate.p.value = TRUE, B = 1000)

#ctable(td$Rule, td$cbrmYN, chisq= TRUE, headings = FALSE) %>%
 # print(method = "render")


```
There is an association between Rule formality and whether or not a household is a member of a CBRM group.
How correllated are Rule formality and cbrm status?  == these are rather correllated, so makes sense that not using both.
[Except that for SPring both are important? but cbrm more in that case]
```{r, echo= FALSE}
...
Plotting correlation matrices for model parameters:
```{r}
library(corrplot)
library(car)
library(ltm)
...
# subset to remove extras:
td.sub <- td[,c(7,10, 13, 17:21)]
...
cortest.bartlett(hetcor(td.sub)$cor, n = nrow(td.sub)) 
