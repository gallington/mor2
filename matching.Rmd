...
hhmatch %<>% dplyr::rename(ez1_DS = dummy_1,
                ez2_St = dummy_2,
                ez3_ES = dummy_3,
                ez4_FMS = dummy_4)
...
```{r P4, }
# where remove NAs get 121 obs for this analysis:
...
a<- bal.tab(p4.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
...
a<- (a$Balance)
# 
b<- bal.plot(p4.out1, "distance", which = "both")+ theme_bw()+ggtitle("Fall Otor") 
# this returns a ggplot object
b# so just need to adjust the aesthetics.
#write.csv(a, file = "./exports_tables/FOtor_balance.csv")
ggsave(b, filename = "./plots/FOtor_balance.png")
...
a<- bal.tab(p5.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
# 
b<- bal.plot(p5.out1, "distance", which = "both")+ theme_bw()+ggtitle("Winter Otor")
# this returns a ggplot object
b # so just need to adjust the aesthetics.
ggsave(b, filename = "./plots/WOtor_balance.png")
#write.csv(a$Balance, file= "./exports_tables/WOtor_balance.csv")
```
...
a<- bal.tab(p6.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
# 
b<- bal.plot(p6.out1, "distance", which = "both")+ theme_bw() + ggtitle("Reserve Winter Pastures")
b

ggsave(b, filename = "./plots/ResWPast_balance.png")
write.csv(a$Balance, file= "./exports_tables/ResWPast_balance.csv")
```
...
a<- bal.tab(p7.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
a
# 
b<- bal.plot(p7.out1, "distance", which = "both")+ theme_bw()+ggtitle("Reserve Spring Pastures")
b
ggsave(b, filename = "./plots/ResSpPast_balance.png")
#write.csv(a$Balance, file= "./exports_tables/ResSpPast_balance.csv")
...
a<-bal.tab(p8.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
a
# 
b<- bal.plot(p8.out1, "distance", which = "both")+ theme_bw() +ggtitle("Reserve Dzud Pastures")
b
ggsave(b, filename = "./plots/ResDzPast_balance.png")
#write.csv(a$Balance, file= "./exports_tables/ResDzPast_balance.csv")
...
a<- bal.tab(p9.out1, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
a
# 
b<- bal.plot(p9.out1, "distance", which = "both")+ theme_bw() + ggtitle("Grazed Reserve Pastures Out-of-Season")

b
ggsave(b, filename = "./plots/GzResPast_balance.png") #, width=2, height = 7)
write.csv(a$Balance, file = "./exports_tables/GzResPast.csv")
...
## RULES :::
```{r Rules, eval = FALSE, echo = FALSE}

raw.data<- mutate(raw.data, r1_No = ifelse(r1 == 0, 1, 0))
raw.data<- mutate(raw.data, r1_Inf = ifelse(r1 == 1, 1, 0))
raw.data<- mutate(raw.data, r1_Form = ifelse(r1 == 2, 1, 0))
raw.data %<>% na.omit()
# No Rules
Norules.full.summary<- summary(No.out <- matchit(r1_No ~
                                          # which covar       
                                          CBRM + 
                                          ez + 
                                          ltmeanppt+
...
# get summary and generate dataset
Norules.full.summary
full.data.Norules <- match.data(No.out) # pulls matched data out of matchit object. can do group = "treat"

# Inf Rules
Infrules.full.summary<- summary(Inf.out <- matchit(r1_Inf ~
                                     # which covar       
                                       CBRM + 
                                        ez + 
                                        ltmeanppt+
...
# get summary and generate dataset
Infrules.full.summary
full.data.Infrules <- match.data(Inf.out) # pulls matched data

# Formal
Formrules.full.summary<- summary(Form.out <- matchit(r1_Form ~
                                     # which covar       
                                       CBRM + 
                                        ez + 
                                        ltmeanppt+
...
# get summary and generate dataset
Formrules.full.summary
full.data.Formrules <- match.data(Form.out) # pulls matched data



# balanace.plot

love.plot(bal.tab(No.out), 
          stat =  "mean.diffs",   #c("mean.diffs", "variance.ratios", "ks.statistics"), 
          # cant' get var ratios for binary vars... can't get this to work unless select one stat msr at a time.
          threshold = .2,  #c(.2, 2), 
          no.missing = TRUE,
...
bal.tab(No.out, # the matchit object
        un = TRUE,  # print stats for unadjusted and adjusted sample?
        m.threshold = 0.2, #.1, # threshold for mean diffs. they suggest .1 which is more conservative than our 0.25 that we were using before
        int = FALSE, #include interactions?
        disp.means = TRUE,
...
# 
b<- bal.plot(No.out, "distance", which = "both")
...
```{r}
# No Rules:
No_0<-(full.data.Norules$eco1.rs*full.data.Norules$weights)[which(full.data.Norules$r1_No==0)]
No_0pre<-(full.data.Norules$eco1.rs)[which(full.data.Norules$r1_No==0)]
No_1<-(full.data.Norules$eco1.rs*full.data.Norules$weights)[which(full.data.Norules$r1_No==1)]
No_1pre<-(full.data.Norules$eco1.rs)[which(full.data.Norules$r1_No==1)]
plot(ecdf(No_0))
lines(ecdf(No_1), col = 'blue')

ks.test(No_0, No_1)
t.test(No_0, No_1)
ks.test(No_0pre, No_1pre)
t.test(No_0pre, No_1pre)

# Informal Rules
Inf_0<-(full.data.Infrules$eco1.rs*full.data.Infrules$weights)[which(full.data.Infrules$r1_Inf==0)]
Inf_0pre<-(full.data.Infrules$eco1.rs)[which(full.data.Infrules$r1_Inf==0)]
Inf_1<-(full.data.Infrules$eco1.rs*full.data.Infrules$weights)[which(full.data.Infrules$r1_Inf==1)]
Inf_1pre<-(full.data.Infrules$eco1.rs)[which(full.data.Infrules$r1_Inf==1)]

# Formal Rules
Form_0<-(full.data.Formrules$eco1.rs*full.data.Formrules$weights)[which(full.data.Formrules$r1_Form==0)]
Form_0pre<-(full.data.Formrules$eco1.rs)[which(full.data.Formrules$r1_Form==0)]
Form_1<-(full.data.Formrules$eco1.rs*full.data.Formrules$weights)[which(full.data.Formrules$r1_Form==1)]
Form_1pre<-(full.data.Formrules$eco1.rs)[which(full.data.Formrules$r1_Form==1)]

# plotting a few:
ggplot(full.data.Norules, aes(x=(eco1.rs*weights), colour=r1_No)) +  geom_density() 


```
#### EF 2:
```{r}
ef2No_0<-(full.data.Norules$eco2.rs*full.data.Norules$weights)[which(full.data.Norules$r1_No==0)]
ef2No_0pre<-(full.data.Norules$eco2.rs)[which(full.data.Norules$r1_No==0)]
ef2No_1<-(full.data.Norules$eco2.rs*full.data.Norules$weights)[which(full.data.Norules$r1_No==1)]
ef2No_1pre<-(full.data.Norules$eco2.rs)[which(full.data.Norules$r1_No==1)]
plot(ecdf(ef2No_0))
lines(ecdf(ef2No_1), col = 'blue')

ks.test(ef2No_0, ef2No_1)


# Informal Rules
ef2Inf_0<-(full.data.Infrules$eco2.rs*full.data.Infrules$weights)[which(full.data.Infrules$r1_Inf==0)]
ef2Inf_0pre<-(full.data.Infrules$eco2.rs)[which(full.data.Infrules$r1_Inf==0)]
ef2Inf_1<-(full.data.Infrules$eco2.rs*full.data.Infrules$weights)[which(full.data.Infrules$r1_Inf==1)]
ef2Inf_1pre<-(full.data.Infrules$eco2.rs)[which(full.data.Infrules$r1_Info==1)]

# Formal Rules
ef2Form_0<-(full.data.Formrules$eco2.rs*full.data.Formrules$weights)[which(full.data.Formrules$r1_Form==0)]
ef2Form_0pre<-(full.data.Formrules$eco2.rs)[which(full.data.Formrules$r1_Inf==0)]
ef2Form_1<-(full.data.Formrules$eco2.rs*full.data.Formrules$weights)[which(full.data.Formrules$r1_Form==1)]
ef2Form_1pre<-(full.data.Formrules$eco2.rs)[which(full.data.Formrules$r1_Inf==1)]
...
ggplot(full.data.p4, aes(x=(eco1.rs), colour=as.factor(p4))) + 
  ...
  geom_vline(aes(xintercept = 0.64), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.69), color = line2, linetype = 3)+
  labs(title="Effect of Fall Otor\npre-matched data", x= "Ecological Factor 1")
...
ggplot(full.data.p4, aes(x=(eco1.rs *weights), colour=as.factor(p4))) + 
  ...
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.64), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.17), color = line2, linetype = 3)+
  labs(title="Effect of Fall Otor\npost-matched data", x= "Ecological Factor 1")
...
ggplot(full.data.p4, aes(x=(eco2.rs), colour=as.factor(p4))) + 
  ...
  geom_vline(aes(xintercept = 0.64), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.69), color = line2, linetype = 3)+
  labs(title="Effect of Fall Otor\npre-matched data", x= "Ecological Factor 2")
...
ggplot(full.data.p4, aes(x=(eco2.rs *weights), colour=as.factor(p4))) + 
  ...
  labs(title="Effect of Fall Otor\npost-matched data", x= "Ecological Factor 2")
...
ggplot(full.data.p5, aes(x=(eco1.rs *weights), colour=as.factor(p5))) + 
  ...
  theme(legend.position="none") +
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.60), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.295), color = line2, linetype = 3)+
...
ggplot(full.data.p5, aes(x=(eco2.rs *weights), colour=as.factor(p5))) + 
  ...
  theme(legend.position="none") +
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.37), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.21), color = line2, linetype = 3)+
  labs(title="Effect of Winter Otor\npost-matched data", x= "Ecological  2")
...
ggplot(full.data.p6, aes(x=(eco1.rs *weights), colour=as.factor(p6))) + 
  ...
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.64), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.31), color = line2, linetype = 3)+
...
ggplot(full.data.p6, aes(x=(eco2.rs *weights), colour=as.factor(p6))) + 
  ...
  theme(legend.position="none") +
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.47), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.17), color = line2, linetype = 3)+
  labs(title="Effect of Reserving Winter Pastures\npost-matched data", x= "Ecological Factor 2")
...
ggplot(full.data.p7, aes(x=(eco1.rs *weights), colour=as.factor(p7))) + 
  ...
  geom_vline(aes(xintercept = 0.32), color = line2, linetype = 3)+
  labs(title="Effect of Reserving Spring Pastures\npost-matched data", x= "Ecological 1")
...
ggplot(full.data.p7, aes(x=(eco2.rs *weights), colour=as.factor(p7))) + 
  ...
  theme(legend.position="none") +
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.36), color = line1, linetype = 2)+
...
ggplot(full.data.p8, aes(x=(eco2.rs *weights), colour=as.factor(p8))) + 
  ...
  geom_vline(aes(xintercept = 0.25), color = line2, linetype = 3)+
  labs(title="Effect of Reserving Dzud Pastures\npost-matched data", x= "Ecological Factor 2")
...
ggplot(full.data.p9, aes(x=(eco1.rs *weights), colour=as.factor(p9))) + 
  ...
  scale_color_brewer(type = 'div', palette = pal1)+
  geom_vline(aes(xintercept = 0.66), color = line1, linetype = 2)+
  geom_vline(aes(xintercept = 0.46), color = line2, linetype = 3)+
...
Before matching:
```{r}
cbrm_mod <- lm(eco1.rs ~ CBRM + ppt99 +sfu99 + as.factor(ez) - 1, data= raw.data ) # force no intercept
summary(cbrm_mod) # No difference
```


```{r Models}
#cbrm_mod <- lm(eco.int ~ CBRM , data= full.data.m, weights = full.data.m$weights)
cbrm_mod <- lm(eco1.rs ~ CBRM + ppt99 +sfu99 + as.factor(ez), data= full.data.m, weights = full.data.m$weights) # force 
summary(cbrm_mod)

# why is this coefficient for CBRM1 negative?
```

#subset the control : 

```{r}
full.data.sub<- full.data.m[which((full.data.sub$eco1.rs*full.data.sub$weights) <= "2"), ]
cbrm_mod_sub <- lm(eco1.rs ~ CBRM+ ppt99 +sfu99 + as.factor(ez), 
                 data= full.data.sub, 
                 weights = full.data.sub$weights,
                 family = gaussian)
summary(cbrm_mod_sub)
```
...
```{r}
zelig(eco1.rs ~ CBRM + ppt99 +sfu99 + as.factor(ez), data = full.data.m, weights = "weights", model = "ls")
...
```{r}
zelig(eco1.rs ~ CBRM + ppt99 +sfu99 + as.factor(ez)+ ltmeanppt, data = full.data.m, weights = "weights", model = "ls")
```

#### Boxplot of post-matched Ecological Index Value by Practice
```{r}
p4ew<- (full.data.p4$eco1.rs*full.data.p4$weights)

p5ew<- (full.data.p5$eco1.rs* full.data.p5$weights)

p6ew<- (full.data.p6$eco1.rs *full.data.p6$weights)
p7ew<- (full.data.p7$eco1.rs  * full.data.p7$weights)
p8ew<-  (full.data.p8$eco1.rs * full.data.p8$weights)
p9ew <- (full.data.p9$eco1.rs * full.data.p9$weights)


ecopr<- list(p4ew, p5ew, p6ew, p7ew, p8ew, p9ew)
ep<-lapply(ecopr, `length<-`, 116)
ecopr<- as.data.frame(ep[[1]], ep[[2]], ep[[3]])


# names(ecopr)<- c("p4", "p5", "p6", "p7", "p8", "p9")
# tbl_df(ep)%>%
# ecopr<- gather(ecopr, Trt, Index, )

width <- 4
height <- 4
ggplot(full.data.p4)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p4))+
  ylim(c(0,2))+
  labs(main = "Fall Otor") +
  theme_bw()
ggsave(file = "./plots/p4box.png", width = width, height = height)                                  
ggplot(full.data.p5)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p5))+
  ylim(c(0,2))+
   labs(main = "Winter Otor") +
  theme_bw()
ggsave(file = "./plots/p5box.png", width = width, height = height)   

ggplot(full.data.p6)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p6))+
  ylim(c(0,2))+
   labs(main = "Res Winter Past") +
  theme_bw()
ggsave(file = "./plots/p6box.png", width = width, height = height)   

ggplot(full.data.p7)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p7))+
  ylim(c(0,2))+
   labs(main = "Res Spring") +
  theme_bw()
ggsave(file = "./plots/p7box.png", width = width, height = height)   

ggplot(full.data.p8)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p8))+
  ylim(c(0,2))+
   labs(main = "Res Dzud") +
  theme_bw()
ggsave(file = "./plots/p8box.png", width = width, height = height)   

ggplot(full.data.p9)+
  geom_boxplot(aes(y= (eco1.rs *weights), x = p9))+
  ylim(c(0,2))+
  labs(main = "Graze Out-of\n Season") +
  theme_bw()
ggsave(file = "./plots/p9box.png", width = width, height = height)   
```
