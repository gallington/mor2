...
for(level in unique(lvs$ez)){
  lvs[paste("dummy", level, sep = "_")] <- ifelse(lvs$ez == level, 1, 0)
}
lvs %<>% rename(ez1_DS = dummy_1,
                ez2_St = dummy_2,
                ez3_ES = dummy_3,
                ez4_FMS = dummy_4)
...
hhmatch %<>% left_join(lvs[,c(2, # RefNum
                              ...
                              57:60 # ecological zone dummy vars
                              )], by= "RefNum")
...
**Table 1.** A full list of variables used in this analysis. (See Table 1 in the word & excell docs too). This will eventually also contain descriptios of the variables and the units and source. 
...
While mean values for ecological condition were not difference for the Trt and control across all comparisons, all except for Grazing Winter Pasture Out of Season resulted in significantly different distributions between the Treatment and Control groups, with significantly higher median values for Ecological Condition for the Treatment groups. 
...
x<- bind_cols(cbrm_mu, cbrm_med)%>% rename(Trt = CBRM)
